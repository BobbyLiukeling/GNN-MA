...
EPOCHS = 20
LR = 1e-3                        # 建议先提高一点（你现在明显卡住）
WEIGHT_DECAY = 1e-4
DROPOUT = 0.2                    # 0.5 可能太强，先降一点

WARMUP_EPOCHS = 2                # 先缩短 warmup
LAMBDA_RANK_AFTER = 0.05
RANK_K_NEG = 10

# Hard negative mining：建议先关掉，等 AUC 能动了再开
USE_HARD_NEG = False
...
# Output / Saving
# =========================
BEST_BY = "ef"          # choose best model by: "ef" (Val EF@1%) or "auc" (Val AUC)
...
class PairGraphDataset(Dataset):
    def __init__(self, pair_list: List[Dict[str, Any]]):
    ...
    def __getitem__(self, idx: int):
        ...
        node1 = p["lig_node"]
        adj1  = p["lig_adj"]
        e1    = p["lig_edge"] if p["lig_edge"] is not None else adj1[..., None]

        # Windows 友好：每次 np.load 用 with 自动关闭，避免句柄泄漏
        with np.load(p["file"]) as dat:
            node2 = dat["node_feat"]
            adj2  = dat["adj"]
            e2    = dat["edge_feat"] if "edge_feat" in dat else adj2[..., None]
        ...
        meta  = {"idx": int(p["__idx"]), "target": p["target"], "file": p["file"], "label": int(p["label"])}
...
# Build pairs
# =========================
def get_graph_pairs_from_target(target_dir: str, mode: str, target_name: str):
    folder = os.path.join(target_dir, mode)
    ligand_file = os.path.join(folder, "crystal_ligand.npz")

    # ligand 只读一次进内存
    with np.load(ligand_file) as ligand:
        lig_node = ligand["node_feat"].astype(np.float32)
        lig_adj  = ligand["adj"].astype(np.float32)
        lig_edge = ligand["edge_feat"].astype(np.float32) if "edge_feat" in ligand else None
    ...
    pos_pairs = [{"target": target_name, "file": f, "label": 1, "lig_node": lig_node, "lig_adj": lig_adj, "lig_edge": lig_edge} for f in act_files]
    neg_pairs = [{"target": target_name, "file": f, "label": 0, "lig_node": lig_node, "lig_adj": lig_adj, "lig_edge": lig_edge} for f in dec_files]
...
def build_dataset(encode_root: str, mode: str, split_ratio=(0.8, 0.1, 0.1)):
    ...
    for t in all_targets:
        ...
        if not os.path.exists(os.path.join(target_dir, mode)):
            continue
        all_pairs.extend(get_graph_pairs_from_target(target_dir, mode, target_name=t))
...
def predict_records(dl, model, device, threshold: float = 0.5):
    """Return per-pair predictions for a dataloader."""
    model.eval()
    records = []
...
def mine_hard_negative_scores(model, train_ds: PairGraphDataset, device, mining_dl_kwargs, top_frac=0.02, max_neg_to_score=100000):
    ...
    # ✅ 修复：不要再额外传 shuffle=False（mining_dl_kwargs 里已经有）
...
def train_model(model, train_pairs, val_pairs, test_pairs, device, run_dir: str):
    train_ds = PairGraphDataset(train_pairs)
    val_ds   = PairGraphDataset(val_pairs)
    test_ds  = PairGraphDataset(test_pairs)
    ...
    for epoch in range(EPOCHS):
        ...
        # hard neg（可选）
        if USE_HARD_NEG and (not in_warmup) and ((epoch + 1) % HARD_NEG_UPDATE_EVERY == 0):
            hard_scores = mine_hard_negative_scores(
                model, train_ds, device,
                mining_dl_kwargs=mining_dl_kwargs,
                top_frac=HARD_NEG_TOP_FRAC,
                max_neg_to_score=HARD_NEG_MAX_NEG_TO_SCORE
            )
        ...
        if is_best:
            print(f"  -> Saved best model to: {best_ckpt_path}", flush=True)
    ...
    append_line(epoch_log_path, f"{ts}\tBEST_TEST\tAUC={auc_test:.6f}\tEF@1%={ef_test:.6f}\tpos={tpos}\tneg={tneg}")

    records = predict_records(test_dl, model, device, threshold=float(PRED_THRESHOLD))
    save_predictions_csv(records, best_test_csv_path)
    print(f"  -> Saved best-model test predictions CSV to: {best_test_csv_path}", flush=True)
    append_line(epoch_log_path, f"{now_str()}\tSaved best test predictions CSV: {best_test_csv_path}")
...
def main():
    ...
    encode_root = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "data", "encode"))
    ...
    node_dim = train_pairs[0]["lig_node"].shape[-1]
    edge_dim = train_pairs[0]["lig_edge"].shape[-1] if train_pairs[0]["lig_edge"] is not None else 1
