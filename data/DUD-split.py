...
def split_list(items: List[Path], seed: int) -> Tuple[List[Path], List[Path], List[Path]]:
    ...
    # 容错：确保 val/test 不为空（只在 n 足够大时处理）
    if n >= 3:
        if len(val) == 0:
...
def process_one_target(target_dir: Path) -> dict:
    """
    处理一个 target：读取 edge_no_sidechain，按 candidate 分层 8:1:1 划分并写出文件
    返回 meta 统计信息（用于汇总报告）
    """
    ...
    # 分层：pos/neg 分别切 8/1/1
    pos_tr, pos_va, pos_te = split_list(pos_files, seed=SEED + hash(target) % 100000 + 1)
    neg_tr, neg_va, neg_te = split_list(neg_files, seed=SEED + hash(target) % 100000 + 2)
    ...
    write_rel_list(neg_tr, out_root / "decoys_train.txt", edge_dir)
    write_rel_list(neg_va, out_root / "decoys_val.txt",   edge_dir)
    write_rel_list(neg_te, out_root / "decoys_test.txt",  edge_dir)
    ...
    meta = {
        ...
        "note": "Candidate molecule-disjoint split (query ligands may be reused across splits; candidates are disjoint).",
...
def main():
    ...
    # 遍历所有 target：要求存在 edge_no_sidechain
    target_dirs = []
