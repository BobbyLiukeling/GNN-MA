...
Explanation:
- Adapt to new datasets where "each target has multiple ligands/actives/decoys";
- For each target: generate positive examples by pairing each ligand with each active; generate negative examples by pairing each ligand with each decoy;
- Compatible directories：
    {ENCODE_ROOT}/{target}/{MODE}/ligand/*.npz   or ligands/*.npz
    {ENCODE_ROOT}/{target}/{MODE}/active/*.npz
    {ENCODE_ROOT}/{target}/{MODE}/decoy/*.npz
  Also compatible with legacy: {MODE}/crystal_ligand*.npz (single or multiple)
'''
import os
import random
import numpy as np
...
class PairGraphDataset(Dataset):
    ...
    The pair is structured as a dict：
      {
        'target':  target name,
        'file':    path to another molecule's .npz file (active/decoy),
        'label':   0/1,
        'lig_node','lig_adj','lig_edge': graph of the current ligand (left graph),
        'lig_file': file path of the current ligand,
        'pair_type': 'pos' or 'neg'
      }
    """
...
# ========= 2. Pair Collection and Division (Multi-ligand Adaptation)  =========
...
def _collect_ligand_files(folder):
    """
    First, look for .npz files in {folder}/ligand or {folder}/ligands.
    If not found, fall back to {folder}/crystal_ligand*.npz (compatible with legacy version).
    """
    ...
    # Compatible with legacy naming: place crystal_ligand*.npz directly in the folder (could be 1 or multiple)
    legacy = []
...
def get_graph_pairs_from_target_multi(
    ...
    max_pos_per_ligand=None,         # Optional: limit the maximum number of positive examples per ligand
    max_neg_per_ligand=None          # Optional: limit the maximum number of negative examples per ligand
):
    """
    Perform full pairing (or limited pairing) of "each ligand × each active/decoy" under a single target.
    """
    ...
    if len(lig_files) == 0:
        # Neither the ligand directory nor legacy files exist, skip this target
        return []

    all_pairs = []
    # Read each ligand once and reuse its numpy array (to avoid repeated loading)
    for lf in lig_files:
        lig_npz = np.load(lf)
        ...
        # Select active/decoy list (optional throttling)
...
def build_dataset(
    ...
    max_pairs_per_target=None,       # Optional: upper limit truncation for "all pairs generated by a single target"
    max_pos_per_ligand=None,         # Optional: upper limit of positive examples per ligand
    max_neg_per_ligand=None          # Optional: upper limit of negative examples per ligand
):
    """
    Summarize all pairs of targets and randomly split into train/val/test
    """
...
def train_model(model, train_dl, val_dl, test_dl, device, out_model_prefix, epochs=20, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_dl:
            ...
            # Simple progress output (once every ~1000ms)
    ...
    print(f"AUC and training time per epoch have been saved: {auc_file}")
    ...
    with torch.no_grad():
        for batch in test_dl:
            ...
            # Write each sample into the list
...
def main():
    ...
    # —— Note: If the data is very large, you can enable throttling parameters —— #
    train, val, test = build_dataset(
        ENCODE_ROOT, MODE,
        split_ratio=(0.8, 0.1, 0.1),
        max_pairs_per_target=100,     # e.g., 50000
        max_pos_per_ligand=100,       # e.g., 200
        max_neg_per_ligand=100        # e.g., 800
    )
    ...
    if len(train) == 0:
        raise RuntimeError("The training set is empty, please check the data directory and naming.")
